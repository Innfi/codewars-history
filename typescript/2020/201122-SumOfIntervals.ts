...
const sumOfIntervals = (intervals: number[][]): number => {
    ...
    let secondFiltered: Interval[] = [];
    filteredIntervals.forEach((value: Interval) => {
        let isOverlapped: boolean = false;
        for(let i=0;i<secondFiltered.length;i++) {                
            if(secondFiltered[i].isOverlapping(value.start, value.end)) {
                secondFiltered[i].overlap(value.start, value.end);
                isOverlapped = true;
                break;
            }
        }
        if(!isOverlapped) secondFiltered.push(value);
    });
