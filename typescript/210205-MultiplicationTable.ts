...
const multiplicationTable = (size: number): number[][] => {
    return Array.from({length: size}, (v, k) => k)
        .map((value: number) => {
            return Array.from({length: size}, (v, k) => (value+1) * (k+1));
        });
}
...
describe('Building blocks', () => {
    ...
    it('write functional style iteration', () => {
        let size: number = 2;

        const result = Array.from({length: size}, (v, k) => k)
        .map((value: number) => {
            return Array.from({length: size}, (v, k) => (value+1) * (k+1));
        });

        assert.deepStrictEqual(result, [[1,2], [2,4]]);
    });
});
